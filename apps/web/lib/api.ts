...
export async function streamChat(messages: Message[], userId?: string): Promise<Response> {
...
export async function triggerDD(company: string, context?: string, userId?: string): Promise<{ ddJobId: string }> {
  const res = await fetch(`${BACKEND_URL}/dd`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ company, context, userId }),
  })
  return res.json()
}
